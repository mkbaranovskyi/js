...
`focus` and `blur` are the events we need. Also there're methods `focus()` and `blor()` to set and unset focus on the element. They don't bubble. We can use `focusin` and `focusout` events instead - they do bubble.
