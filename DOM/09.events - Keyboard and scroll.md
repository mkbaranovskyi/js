...
- [Keyboard events](#keyboard-events)
	- [`e.code` and `e.key`](#ecode-and-ekey)
	- [Repeat](#repeat)
	- [Utility keys](#utility-keys)
	- [Task: Extended hotkeys](#task-extended-hotkeys)
- [Scroll events](#scroll-events)
	- [Load visible images](#load-visible-images)

## `e.code` and `e.key`

```javascript
document.addEventListener('keydown', e => {
	// press `z` and then 'shift' + `z`
...
![](img/2020-03-24-16-28-31.png)

Note that `e.code` is the same for 'z' and 'Z' but `e.key` is different. Even pressing the key with another layout (in another language) will result in the same `KeyZ` result for `e.code`. 


## Repeat

If we press and hold a key, it will trigger multiple `keydown` events and only one `keyup`. And for such events `e.repeat` prop becomes `true` (while the first one is `false`).

## Utility keys

All of them might not work everywhere. Return booleans.
...
document.addEventListener('keypress', e => {
	...
	console.log('Meta: ', e.metaKey)		// `Windows` / Mac `Command`
	console.log('\n')
...
Write a function that adds handlers for running the provided function upon certain keys pressing.
...
function runOnKeys(func, ...codes){
	...
	document.addEventListener('keydown', e => {
		...
		pressed.clear()		// safery measure from 'sticky' keys during alert
...
https://javascript.info/onscroll#load-visible-images

<a href="loadVisibleImages/index.html">Full solution</a>
...
<body>
	...
	<script>
		...
		function showImages(){
			...
			const coords = images.map(img => img.getBoundingClientRect())

			coords.forEach((coord, index) => {
				// either top of the img is visible (+- 1 page), or bottom (+-1 page), or top is higher and bottom is lower (in case of a gigantic picture)

				if(coord.top < window.innerHeight * 2 && coord.top > -window.innerHeight ||
				coord.bottom > -window.innerHeight && coord.bottom < window.innerHeight * 2 ||
				coord.top < 0 && coord.bottom > window.innerHeight){
					images[index].src = images[index].dataset.src + '?nocache=' + Math.random() // the ending is for the testing purposes only
