...
`I`: Вместо того, чтобы создавать интерфейс, в котором лежат все возможные поля, и помечать эти поля как опциональные, лучше создать несколько раздельных интерфейсов - каждый для своих задач.

`D`: Важные высокоуровневые модули (напр. контроллеры) не должны зависеть от изменений в маленьких низкоуровневых модулях (напр. смс-сервис, работа с токенами и т.д.). Т.е. мы должны быть способны безболезненно заменить низкоуровневые модули (сервисы) другими, не мешая работе высокоуровневых контроллеров. Это достигается созданием абстрактных интерфейсов для связи между ними. 
