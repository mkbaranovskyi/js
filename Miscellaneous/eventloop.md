...
- When a user moves their mouse, the task is to dispatch a mousemove event and execute handlers.
...
Stack represents the only execution thread present in JS. When a function executes, expressions from the function get into the Stack, execute and then pop out. When the Stack is **completely empty**, only then the Queue proceeds to the next task.
...
let tasks, microtasks, renderQueue, stack
