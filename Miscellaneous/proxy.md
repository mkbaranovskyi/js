...
```javascript
var target = {}		// empty object
var proxy = new Proxy(target, {})	// empty handler, no restrictions
...
## [Iteration with “ownKeys” and “getOwnPropertyDescriptor”](<a name="iteration">)
...
## [Protected properties with “deleteProperty” and other traps](<a name="delete">)
...
user = new Proxy(user, {
	...
	deleteProperty(target, prop){	// a trap for `delete` operator
		if(prop.startsWith('_')){
			throw new Error('Access denied')
		} else {
			delete target[prop]
...
## [Wrapping functions: "apply"](<a name="wrapping-functions">)
...
const Func = new Proxy(function() {}, {
	construct(target, args){
		return {	// `construct` should return an object, we can do it explicitly
...
## [defineProperty](<a name="define-property>)
...
## [getOwnPropertyDescriptor](<a name="get-descr">)
...
## [Revocable proxy](<a name="revocable-proxy">)
